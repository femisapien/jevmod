...
class Svg:
    ...
    def text(self, x: float, y: float, s: str, size: int = 15, fill: str = FG, mono: bool = False,
    ...
    def box(self, x: float, y: float, w: float, h: float, title: str, sub: str = "", lines: list[str] | None = None,
            hot: bool = False, mono_title: bool = False, dashed: bool = False) -> None:
    ...
    def arrow(self, x1: float, y1: float, x2: float, y2: float, accent: bool = False, dashed: bool = False,
              label: str = "", label_dy: float = -8) -> None:
...
def architecture() -> None:
    ...
    s.text(32, 68, "Every surface is an adapter over one service. Only the text and the channel topic reach Jev.", 15, FG2)
    ...
    for i, (name, how) in enumerate(chans):
        ...
        s.parts.append(f'<rect x="{x}" y="{y}" width="136" height="50" rx="6" fill="{BG2}" stroke="{LINE2}" stroke-width="1.5"/>')
    ...
    s.box(380, 176, 230, 156, "ModerationService", "jevmod.core", [
    ...
    s.box(672, 176, 150, 156, "Judge", "jevmod.judge", [
        "normalise text",
        "prefilter, cache",
        "batch of messages",
        "one request",
    ...
    # store on the side
    s.path(f"M495,336 L495,372", arrow=True)
    s.box(380, 376, 230, 96, "Store", "SQLite on a volume", [
        "policy, usage, audit log",
        "300 chars of text, 30 days",
    ])
    s.text(32, 520, "categories.json holds the questions asked of Jev, so Python, npm and MCP ask exactly the same thing.", 13, FG3)
    s.write("architecture.svg")


def flow() -> None:
    s = Svg(1000, 500, "jevmod request flow: prefilter, cache, batch, one Jev request, probabilities, policy, action, audit log")
    s.text(32, 40, "request flow", 13, FG3, mono=True)
    s.text(32, 68, "What happens to one message. Anything the prefilter or the cache answers never leaves the machine.", 15, FG2)

    def node(x: float, y: float, w: float, title: str, sub: str, hot: bool = False, dashed: bool = False) -> None:
        dash = ' stroke-dasharray="6 5"' if dashed else ""
        fill = BG if dashed else BG2
        s.parts.append(f'<rect x="{x}" y="{y}" width="{w}" height="64" rx="6" fill="{fill}" stroke="{ACCENT if hot else LINE2}" stroke-width="1.5"{dash}/>')
        s.text(x + 14, y + 26, title, 15, FG, weight="600")
        s.text(x + 14, y + 47, sub, 11.5, FG3, mono=True)

    # row 1, left to right
    y1 = 104
    row1 = [(32, 140, "message", "text, topic"), (194, 176, "prefilter", "8+ letters, untrusted"),
            (392, 150, "cache", "same text, reuse"), (564, 160, "batch", "2 s, up to 50 msgs"),
    ...
    s.parts.append(f'<rect x="500" y="{y2}" width="436" height="64" rx="6" fill="{BG}" stroke="{ACCENT}" stroke-width="1.5" stroke-dasharray="6 5"/>')
    ...
    y3 = 356
    row3 = [(746, 190, "probabilities", "one per category"), (554, 170, "policy", "thresholds, rules"),
            (342, 190, "action", "flag, delete, timeout"), (140, 180, "audit log", "probabilities + action")]
    ...
    s.text(32, 462, "Measured on the benchmark: 1,005 input tokens per judged message with seven categories, 22 ms per message in batches of 25.", 13, FG3)
    s.write("flow.svg")
...
def coverage() -> None:
    ...
    cats = [
        ("spam", "flag 0.85"), ("scam", "flag 0.75"), ("harassment", "flag 0.75"), ("nsfw", "flag 0.80"),
        ("offtopic", "off, needs topic"), ("selfharm", "flag 0.80, never punish"), ("doxxing", "flag 0.80"),
        ("minors", "flag 0.70"), ("rule:<name>", "flag 0.80, up to 5"),
    ]
    ...
    s.text(32, 68, "One set of questions (categories.json) on every surface. Default action and threshold in the second column.", 15, FG2)
    ...
    for i, (cat, default) in enumerate(cats):
        ...
        for j in range(len(surfaces)):
            cx = x0 + j * cw + cw / 2
            if cat == "offtopic":
                s.parts.append(f'<rect x="{cx - 6}" y="{y + 11}" width="12" height="12" rx="2" fill="none" stroke="{ACCENT}" stroke-width="1.5"/>')
            else:
    ...
    s.parts.append(f'<rect x="160" y="{ly - 10}" width="12" height="12" rx="2" fill="none" stroke="{ACCENT}" stroke-width="1.5"/>')
...
def failure() -> None:
    ...
    def lane(y: int, trigger: str, tsub: str, steps: list[tuple[str, str]]) -> None:
        s.parts.append(f'<rect x="32" y="{y}" width="200" height="64" rx="6" fill="{BG2}" stroke="{ACCENT}" stroke-width="1.5"/>')
        ...
        for title, sub in steps:
            ...
            x += 40
            s.parts.append(f'<rect x="{x}" y="{y}" width="200" height="64" rx="6" fill="{BG2}" stroke="{LINE2}" stroke-width="1.5"/>')
            s.text(x + 14, y + 26, title, 15, FG, weight="600")
            s.text(x + 14, y + 47, sub, 11.5, FG3, mono=True)
            x += 200

    lane(104, "Jev unreachable", "timeout, 5xx, network", [
        ("fail open", 'reason "error_open"'),
        ("nothing acted on", "action none"),
        ("one warning per batch", "log channel or stderr"),
    ])
    lane(208, "free quota used up", "5,000 judged a month", [
        ("judging pauses", "messages pass untouched"),
        ("owner told once", "not on every message"),
        ("resumes next month", "or with a paid key"),
    ])
    s.text(32, 306, "Retries: 429 and 529 from Jev are retried with exponential backoff and Retry-After before a batch is declared failed.", 13, FG3)
    s.write("failure.svg")
