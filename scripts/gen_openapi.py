"""Write api/openapi.json from the running FastAPI app, so the published schema cannot drift from the code.

The summary line lists the categories from jevmod/categories.json rather than repeating them by hand, which is
how the published description came to name five of the nine. `tests/test_offline.py` fails when this is out of date.
"""
